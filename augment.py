...
def batch_mixup_seg(images, masks, PROBABILITY=2.0, batch_size=0):
...
def batch_cutmix_seg(images, labels, PROBABILITY=2.0, batch_size=0):
...
def mosaic_seg(x, y, batch_size=0):
    ...
    for i in range(batch_size):
        xr = tf.random.uniform([], 0.4, 0.6)
        yr = tf.random.uniform([], 0.4, 0.6)
...
def mosaic_full_seg(x, y, batch_size=0):
    # input image - is a batch of images of size [n,dim,dim,3] not a single image of [dim,dim,3]
    ...
    x_list, y_list = [], []
    for i in range(batch_size):
        x_center = tf.cast(tf.random.uniform([], 0.4, 0.6) * x.shape[2], tf.int32)
        y_center = tf.cast(tf.random.uniform([], 0.4, 0.6) * x.shape[1], tf.int32)
